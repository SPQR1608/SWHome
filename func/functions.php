...
//led
function changeLedStatus($userID, $ledStatus, $ledName){
    global $bd;
    connectBD();
    $query = "UPDATE leds SET status = '" . $ledStatus . "' WHERE userID='" .$userID /*$_SESSION['user_login'] */. "' and ledName = '".$ledName."'";
    $bd->query($query);
    closeBD();
}

function checkLedStatus($userID){
    global $bd;
    connectBD();
    $query = "SELECT status, ledName FROM leds WHERE userID='" . $userID . "'";
    $result = $bd->query($query);
    closeBD();
    return resultSetToArray($result);
}

function addUser($login, $password, $email, $fName, $lName, $date_rojd)
{
    global $bd;
    connectBD();
    $query = "insert into users(userLogin,userPassword,eMail,firstName,secondName,dateRojd,regDate) values('" . $login . "','" . $password . "','" . $email . "', '". $fName ."', '". $lName ."', '" . $date_rojd . "', CURDATE())";
    $bd->query($query);
    closeBD();
}
...
function getUserID($login){
    global $bd;
    connectBD();
    $query = "SELECT id FROM users WHERE userLogin like '".$login."'";
    $result = $bd->query($query);
    closeBD();
...
function checkAndrUser($login, $password)
{
    global $bd;
    connectBD();
    $query = "SELECT * FROM users WHERE userLogin like '" . $login . "' and userPassword like '".$password."'";
    $result = $bd->query($query);
    closeBD();
    return $result->num_rows;
}

function changeAdrUserStatus($login, $status)
{
    global $bd;
    connectBD();
    $query = "UPDATE users SET andrStatus = '".$status."' WHERE userLogin like '".$login."'";
    $bd->query($query);
    closeBD();
}

function checkAndrUserStatus($login)
{
    global $bd;
    connectBD();
    $query = "SELECT andrStatus FROM users WHERE userLogin like '".$login."'";
    $result = $bd->$query($query);
    closeBD();
    return $result;
}
